...
class CategoryManager<T: Favoritable> {
    ...
    func toggleFavorites(items: inout [T], targetItem: T) {
    ...
    func filteredFavoriteItems(items: [T], flag: Bool = true) -> [T] {
        return items.filter {
                $0.isFavorite == flag
        }
    }
    ...
    func loadFavoriteIds() -> [Int] {
        UserDefaults.standard.array(forKey: storageKey) as? [Int] ?? []
    }
